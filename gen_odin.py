        ...
        try:
            ...
            stack_strings.append(
                [
                    ...
                    f"{path.basename(frame.f_code.co_filename)}:{frame.f_lineno}",
                ]
            )
            i += 1
        except:
...
def strip_circumfix(prefix: str, suffix: str, string: str) -> str:
    ...
    assert stripped != None, (
        f'"{string}" did not have circumfix "{prefix}" and "{suffix}'
    )
    return stripped
...
def peek_named_type(type_desc) -> str:
    if type_desc["kind"] == "Builtin":
        return type_desc["builtin_type"]
    elif type_desc["kind"] == "User":
...
_imgui_bounds_value_overrides = {
    ...
    "(IM_UNICODE_CODEPOINT_MAX +1)/4096/8": str(
        int((IM_UNICODE_CODEPOINT_MAX + 1) / 4096 / 8)
    ),
}
...
def get_array_count(type_desc) -> str:
    ...
    if str_to_int(bounds_value) != None:
        return bounds_value

    if bounds_value in _imgui_bounds_value_overrides:
        return _imgui_bounds_value_overrides[bounds_value]

    if bounds_value in processed_defines:
        return define_strip_prefix(bounds_value)
...
def convert_imvec_value(value: str, components: int) -> str:
    ...
    for i in range(len(values)):
        values[i] = make_value_odiney(
            values[i].strip(), "int"
        )  # We say "int" here to imply that it's numeric
...
def make_value_odiney(value: str, type_hint: str = None) -> str:
    if value == "IM_COL32(255, 0, 0, 255)":
        return "u32(0xff0000ff)"  # joyous world
    if value in _odin_value_aliases:
        return _odin_value_aliases[value]
    if type_hint == "Vec2":
        return convert_imvec_value(value, 2)
    if type_hint == "Vec4":
        return convert_imvec_value(value, 4)
    # TODO: This is almost certainly not the right place to do this
    if strip_imgui_branding(value) != value:
        return strip_imgui_branding(value)
    if value.endswith("f"):
        return value.removesuffix("f")

    if value == "0" and not type_is_int(type_hint):
        return "{}"
...
def append_aligned_field(aligned_fields, field_components, comment_parent):
    ...
    if "attached" in comment:
        aligned_fields.append(field_components + [" " + comment["attached"]])
    else:
        aligned_fields.append(field_components)
...
def _write_aligned_fields_range(file: typing.IO, aligned_fields, indent=0):
    ...
    for field in aligned_fields:
        for component_idx in range(len(field)):
            if component_idx >= len(column_sizes):
                column_sizes.append(0)

            component = field[component_idx]
            column_sizes[component_idx] = max(
                column_sizes[component_idx], len(component)
            )
    ...
    for field in aligned_fields:
        ...
        for component_idx in range(len(field)):
            ...
            if component_idx == len(field) - 1:
                whitespace_amount = 0  # Don't pad last element
...
def write_aligned_fields(file: typing.IO, aligned_fields, indent=0):
    ...
    for field_idx in range(len(aligned_fields)):
        ...
        if type(field) == str:
            ...
            if field_idx > last_non_delimiter:
                ...
                _write_aligned_fields_range(
                    file, aligned_fields[last_non_delimiter:field_idx], indent
                )
    ...
    if len(aligned_fields) > last_non_delimiter:
        _write_aligned_fields_range(
            file, aligned_fields[last_non_delimiter : len(aligned_fields)], indent
        )
...
for define in _defines_to_emit + _allowed_user_defines + _allowed_ifdef:
    if not define in _defines_to_process:
        _defines_to_process.append(define)
...
def _ifdef(define: str) -> bool:
    if not define in _allowed_ifdef:
        die("Not allowed to check define '" + define + "'")
...
def condition_if(expression_str) -> bool:
    ...
    try:
        while len(expression_str) > 0:
            ...
            assert ok
            if _ifdef(found_str) == invert:
                return False
            [expression_str, ok] = _chomp("&&", expression_str)
            if ok:
                assert len(expression_str) > 0
    except:
...
def passes_conditionals(thing_with_conditionals) -> bool:
    if not "conditionals" in thing_with_conditionals:
        return True
    ...
    for conditional in thing_with_conditionals["conditionals"]:
        ...
        if condition_kind == "ifdef":
            if not condition_ifdef(expression):
                return False
        elif condition_kind == "ifndef":
            if condition_ifdef(expression):
                return False
        elif condition_kind == "if":
            if not condition_if(expression):
                return False
        elif condition_kind == "ifnot":
            if condition_if(expression):
                return False
        else:
...
def ingest_and_write_defines(file: typing.IO, defines):
    ...
    for define in defines:
        ...
        if not define_name in _defines_to_process:
            continue

        if define_name in _define_overrides:
            processed_defines[define_name] = _define_overrides[define_name]
        else:
            is_user_define = define["source_location"]["filename"].endswith(
                "imconfig.h"
            )
            ...
            if define_name in processed_defines:
                die(
                    "Define '"
                    + define_name
                    + "' already defined! This is almost certainly not correct"
                )
            else:
        ...
        if define_name in _defines_to_emit:
            append_aligned_field(
                aligned,
                [
                    define_strip_prefix(define_name),
                    f" :: {define.get('content', 'true')}",
                ],
                define,
            )
...
def enum_parse_value(value: str, enum_name, expected_prefix: str) -> str:
    if value.find("<") != -1 or str_to_int(value) != None:
        ...
        for enum in enums:
            element_list.append(
                enum_name + "." + enum_parse_field_name(enum, expected_prefix)
            )
...
def write_enum_as_flags(file, enum, enum_field_prefix, name):
    ...
    for element in enum["elements"]:
        if not passes_conditionals(element):
            continue
        ...
        if bit_index != None:
            ...
            append_aligned_field(
                aligned_enums, [element_name, f" = {bit_index},"], element
            )
            handled = True
        else:
            # We're something weird
            if element_value == "0":
                continue  # This is the _None variant, which can be represented in Odin by {}
            ...
            if try_eval(element_value) != None:
                ...
                append_aligned_field(
                    aligned_flags,
                    [
                        f"{name}_{element_name}",
                        f" :: c.int({element_value}) // {extra_comment}",
                    ],
                    element,
                )
                handled = True
            else:
                flag_combination = ",".join(
                    enum_parse_flag_combination(element_value, enum_field_prefix)
                )
                append_aligned_field(
                    aligned_flags,
                    [f"{name}_{element_name}", f" :: {name}{{{flag_combination}}}"],
                    element,
                )
                handled = True
    ...
    # Flags
    write_line_with_comments(
        file, f"{name} :: bit_set[{name.removesuffix('s')}; c.int]", enum
    )
...
def write_enum_as_constants(file, enum, enum_field_prefix, name):
    ...
    for element in enum["elements"]:
        ...
        if field_value_evald != None:
            append_aligned_field(
                aligned, [f"{name}_{field_name}", f" :: {name}({field_value})"], element
            )
        else:
            ...
            append_aligned_field(
                aligned,
                [f"{name}_{field_name}", f" :: {name}({' | '.join(enums)})"],
                element,
            )
...
def write_enum(file: typing.IO, enum, enum_field_prefix: str, name: str):
    ...
    prev_value = (
        -1
    )  # This is a hack as we elide value expressions if our value == prev_value + 1
    ...
    for element in enum["elements"]:
        ...
        if value_is_implicit:
            ...
            append_aligned_field(
                aligned, [f"{field_name} = {element['value']},"], element
            )
...
def write_enums(file: typing.IO, enums):
    ...
    for enum in enums:
        if not passes_conditionals(enum):
            continue
...
def write_structs(file: typing.IO, structs):
    ...
    for struct in structs:
        ...
        if (
            g_has_imgui_internal_and_is_not_in_imgui_internal
            and entire_name in _imgui_struct_skip_if_not_imgui_internal
        ):
            continue

        if not passes_conditionals(struct):
            continue
        ...
        for field in struct["fields"]:
            if not passes_conditionals(field):
                continue
            name = field["name"]
            ...
            append_aligned_field(
                field_components, [f"{name}: ", f"{field_type},"], field
            )
...
def function_to_string(function, as_type=True) -> str:
    ...
    for argument_idx in range(len(arguments)):
        ...
        if default_value:
            argument_list.append(f"{argument_name}: {argument_type} = {default_value}")
        else:
...
def write_functions(file: typing.IO, functions):
    ...
    for function in functions:
        ...
        if entire_name in _imgui_functions_skip:
            continue
        if not passes_conditionals(function):
            continue
...
def write_typedefs(file: typing.IO, typedefs):
    ...
    for typedef in typedefs:
        if not passes_conditionals(typedef):
            continue
        ...
        if entire_name in _imgui_typedef_overrides:
            append_aligned_field(
                aligned,
                [
                    strip_imgui_branding(entire_name),
                    f" :: {_imgui_typedef_overrides[entire_name]}",
                ],
                typedef,
            )
            continue
        ...
        append_aligned_field(
            aligned,
            [strip_imgui_branding(entire_name), f" :: {parse_type(typedef['type'])}"],
            typedef,
        )
...
def main():
    if not sys.version.startswith("3.11.5"):
        ...
        print(
            f"Warning: odin-imgui has been tested against Python version 3.11.5. (Yours: {sys.version})",
            file=sys.stderr,
        )
    ...
    parser.add_argument(
        "--imgui", required=True, help="The dear_bindings json file for imgui.h"
    )
    parser.add_argument(
        "--imconfig", required=True, help="The dear_bindings json file for imconfig.h"
    )
    parser.add_argument(
        "--imgui_internal", help="The dear_bindings json file for imgui_internal.h"
    )
