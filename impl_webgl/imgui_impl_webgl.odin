...
SetupRenderState :: proc(
    ...
    vertex_array_object: gl.VertexArrayObject,
) {
    bd := GetBackendData()
        ...
        for cmd_i in 0 ..< draw_list.CmdBuffer.Size {
            ...
            if pcmd.UserCallback != nil {
                ...
                gl.DrawElements(
                    gl.TRIANGLES,
                    int(pcmd.ElemCount),
                    size_of(imgui.DrawIdx) == 2 ? gl.UNSIGNED_SHORT : gl.UNSIGNED_INT,
                    cast(rawptr)uintptr(pcmd.IdxOffset * size_of(imgui.DrawIdx)),
                )
            }
        }
    }
